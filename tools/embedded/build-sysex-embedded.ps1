...
# -- Backup retention -------------------------------------------------------
$removed = 0
$removed += Remove-OldBackups -Pattern '^ysfc-sysex-converter\.\d{8}\.\d{6}\.html$' -Keep $BackupsToKeep
$removed += Remove-OldBackups -Pattern '^ysfc-sysex-converter\.\d{8}\.\d{6}\.js$' -Keep $BackupsToKeep
if ($removed -gt 0) {
    Write-Host ""
